...
In simple cases, we have only one random variable. In the example below, we have synthetic data representing grades for a single class. Using this distribution, we can calculate statistics such as the mean or variance. The height of each bar represents the frequency of grades, indicating that roughly 60 people received an A in the class.


<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/math_grades_dist.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 1. Distribution of Math Grades" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>

However, in many real-world cases, we are often interested in the distribution of two or more random variables modeled together. For example, we can ask what the distribution of grades is for two classes.

The distribution of two or more random variables is called the joint distribution, denoted as $f_{X, Y}(x, y)$ for random variables $X$ and $Y$. This definition can be generalized to $n$ amount of random variables, so a joint distribution can look like $f_{X, Y, Z}(x, y, z)$ or even $f_{A, B, C, D}(a, b, c, d)$. 
...
Let's do an example. Let $X$ be the random variable for Math grades and $Y$ be the random variable for English grades. The following graph depicts the distribution of English grades.

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/eng_grades_dist.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 2. Distribution of English Grades" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/tile_plot.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 3. Distribution of Joint Grades" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>
...
To get the marginal distributions, e.g., the distribution of Math grades or the distribution of English grades, we can collapse all dimensions other than the dimension of interest. In mathematical notation it is
...
I often only see examples of joint distributions and marginal distributions using two random variables, which is why I was confused to see that a joint distribution can have more than two marginal distributions. However, we can generalize to $n$ random variables. Let's do a three-dimensional case to illustrate the logic, but the reasoning can be extended to $n$ dimensions.
...
I hope this makes marginal distributions clearer because I know when I was an undergrad, I had a bit of trouble wrapping my head around this.
...
with the p.d.f. of the joint distribution defined as
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/math_problem.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 4. Integration Region" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>

Intuitively, this means that the random variable of $X$ takes values between 1 and 0, while $Y$ takes values between $x^3$ and $\sqrt{x}$. The p.d.f. is defined by the surface 4xy - y^3 and the area under this surface represents probability. 

First, we find the normalizing constant by solving for $C$. We do this by integrating the joint p.d.f. over the support of $X$ and $Y$ and setting the double integral to 1.
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/margin_x.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 5. Marginal Distribution of X" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>

For $f_Y(y)$, we flip the order of integration. To do so, we need to redefine the bounds of the integral. The bounds of $X$ are then 
...
This was sort of a nasty integral, but at least we arrived at the marginal p.d.f. for $Y$. The plot of the distribution is shown below.

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/margin_y.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 6. Marginal Distribution of Y" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>
...
Now that we have a proper joint p.d.f. let's calculate a probability.

Ex 1: Find $P(0 \leq X \leq 0.5, 0 \leq Y \leq 0.5)$.

Answer:

The integral for the joint distribution turns out to be intractable, so we resort to an approximation technique like rejection sampling to solve this problem.
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/joint_dist.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 7. Joint Distribution of X and Y" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/contour.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 8. Contour Plot of Joint Distribution" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>

From both of the plots, we can see that most of the density is near the value of 1. All the points are clustered there, and the height of the p.d.f. is greatest around that point.
