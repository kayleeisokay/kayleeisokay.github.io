...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/bike_hist.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 1. Distribution of Bike Shares" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/bike_post_prior.png" class="img-fluid rounded z-depth-1" 
        caption="Figure 2. Prior and Posterior Distributions" %}
    </div>
    <div class="col-sm mt-3 mt-md-0">
    </div>
</div>
...
There are two common ways to generate an estimate for the parameter $\Theta$. First, we will do the Expected A Posteriori method (EAP).
...
From here, we can make a prediction: the expected number of bike shares per hour given the new data is 393.62 because the expected value of a Poisson distribution with parameter $\lambda$ is simply $\lambda$.
...
$$
\operatorname{argmax}_{x} ~ Gamma(10234, 26) \rightarrow \operatorname{argmax}_{x} ~ \frac{26^{10234}}{\Gamma(10234)} x^{10233}e^{-26x}
$$

Essentially, we take the derivative with respect to $x$ and set it equal to zero. Luckily for us, there is a closed-form solution, namely $\frac{\alpha - 1}{\beta}$. Using this formula, we get the MAP estimate to be

$$
\hat{\Lambda} = \frac{\alpha_{new} - 1}{\beta_{new}} = 393.5769,
$$

which is very similar to the EAP estimate. Using this estimate, we can make a prediction just like before
...
From the output above, we see that the likelihood for $\theta = 10$ is greater than the likelihood value for $\theta = 5$. 
